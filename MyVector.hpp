...
namespace myvec
{
	...
	class MyVector
	{
		...
		public:
			...
			constexpr MyVector(	InputIt first, 
								InputIt last, 
		...
		public:
			...
			MyVector& operator=(const MyVector& copy)
				requires std::copy_constructible<T>
			{
				if (storage_.data_ == copy.storage_.data_)
					return *this;
...
//------------------------------------------------------------------------------------------------
			...
			T*			data		(void)			noexcept { return storage_.data_; }
			const T*	data		(void)	const	noexcept { return storage_.data_; }
...
//------------------------------------------------------------------------------------------------
			...
			{ 
				for (size_t i = 0; i < storage_.size_; ++i)
					StorageRAII::AllocTraits::destroy(storage_.alloc_, storage_.data + i);
		...
		private:
			...
			constexpr void push_back_impl(V&& val)
			{
				if (storage_.size_ == storage_.capacity_)
				{
					...
					StorageRAII::AllocTraits::construct(temp.alloc_, temp.data_ + temp.size_, std::forward<V>(val));
